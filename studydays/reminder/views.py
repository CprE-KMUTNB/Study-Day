...
class ReminderView(APIView):
    ...
    def get(self, request, format = None):
        ...
        serializers = ReminderSerializer(models.Reminder.objects.get_all_reminder(user=id), many = True)
        return Response(serializers.data)

class ReminderManager(APIView):
    def get_object(self, pk):
        try:
            return models.Reminder.objects.get(pk=pk)
        except models.Reminder.DoesNotExist:
            raise Http404
    
    def get(self, request, pk, format=None):

        serializers = ReminderSerializer(self.get_object(pk))
        return Response(serializers.data)
